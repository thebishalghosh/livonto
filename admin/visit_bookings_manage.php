        ...
        if ($action === 'update_status' && isset($_POST['new_status'])) {
            ...
            if (in_array($newStatus, ['pending', 'confirmed', 'cancelled', 'completed'])) {
                try {
                    ...
                    error_log("Error updating visit booking status: " . $e->getMessage());
                    $_SESSION['flash_message'] = 'Error updating visit booking status';
            ...
            try {
                ...
                error_log("Error updating visit booking notes: " . $e->getMessage());
                $_SESSION['flash_message'] = 'Error updating admin notes';
                $_SESSION['flash_type'] = 'danger';
            }
            ...
            try {
                ...
                error_log("Error deleting visit booking: " . $e->getMessage());
                $_SESSION['flash_message'] = 'Error deleting visit booking';
                $_SESSION['flash_type'] = 'danger';
            }
        }
    }
...
try {
    ...
    // Get statistics
    $stats = [
        'total' => $db->fetchValue("SELECT COUNT(*) FROM visit_bookings") ?: 0,
        'pending' => $db->fetchValue("SELECT COUNT(*) FROM visit_bookings WHERE status = 'pending'") ?: 0,
        'confirmed' => $db->fetchValue("SELECT COUNT(*) FROM visit_bookings WHERE status = 'confirmed'") ?: 0,
        'completed' => $db->fetchValue("SELECT COUNT(*) FROM visit_bookings WHERE status = 'completed'") ?: 0,
        'cancelled' => $db->fetchValue("SELECT COUNT(*) FROM visit_bookings WHERE status = 'cancelled'") ?: 0,
        'today' => $db->fetchValue("SELECT COUNT(*) FROM visit_bookings WHERE DATE(created_at) = CURDATE()") ?: 0,
        'this_month' => $db->fetchValue("SELECT COUNT(*) FROM visit_bookings WHERE MONTH(created_at) = MONTH(CURDATE()) AND YEAR(created_at) = YEAR(CURDATE())") ?: 0
    ];
                ...
                <div class="modal-body">
                    ...
                    <form method="POST" action="?action=update_status&id=<?= $booking['id'] ?>" class="mb-4">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="new_status<?= $booking['id'] ?>" name="new_status" required>
